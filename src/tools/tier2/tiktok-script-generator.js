...
export default {
  name: "tiktok_script_generator",
  description: "Generate tiktok script generator content. Returns a prompt for the assistant to execute.",
  inputSchema: {
    "type": "object",
    "properties": {
      "topic": {
  ...
  async run(args) {
    const prompt = `Write a TikTok short-form video script on this topic. Follow the format exactly — no extra commentary, no preamble.
...
- <1 visual for the CTA>`;
    return [
      ...
      "---",
      "",
      `Web version: ${SITE_BASE}/tiktok-tools/tiktok-script-generator/`,
    ].join("\n");
  },
};
