...
export default {
  name: "threads_bio_generator",
  description: "Generate threads bio generator content. Returns a prompt for the assistant to execute.",
  inputSchema: {
    "type": "object",
    "properties": {
      "name": {
  ...
  async run(args) {
    const prompt = `Write 3 distinct Threads bio variations. Each must stay within 150 characters. Each should feel different in approach but match the same tone.
...
Platform rules: Threads bio max is 150 characters. Threads has a conversational, casual culture — skip the corporate-speak. Emojis welcome. Line breaks supported. No hashtags in bio.
...
FORMAT — output exactly 3 variations, each separated by a line with only "---" (three dashes). No preamble, no commentary, no "Variation 1" labels. Just the three bio texts separated by ---`;
    return [
      ...
      "---",
      "",
      `Web version: ${SITE_BASE}/threads-tools/threads-bio-generator/`,
    ].join("\n");
  },
};
