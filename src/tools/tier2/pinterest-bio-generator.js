...
export default {
  name: "pinterest_bio_generator",
  description: "Generate pinterest bio generator content. Returns a prompt for the assistant to execute.",
  inputSchema: {
    "type": "object",
    "properties": {
      "name": {
  ...
  async run(args) {
    const prompt = `Write 3 distinct Pinterest bio variations. Each must stay within 160 characters. Each should feel different in approach but match the same tone.
...
Platform rules: Pinterest bio max is 160 characters. Pinterest is a visual search engine — KEYWORDS matter more than personality. Include the exact topics/niches you pin about. Emojis technically allowed but less common on Pinterest. Avoid hashtags in bio.
...
FORMAT — output exactly 3 variations, each separated by a line with only "---" (three dashes). No preamble, no commentary, no "Variation 1" labels. Just the three bio texts separated by ---`;
    return [
      ...
      "---",
      "",
      `Web version: ${SITE_BASE}/pinterest-tools/pinterest-bio-generator/`,
    ].join("\n");
  },
};
